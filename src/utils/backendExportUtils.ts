// src/utils/backendExportUtils.ts
import { apiRequest } from '@/lib/api/apiClient';

// Simplified to one export type that includes all formats
export type ExportFormat = 'pdf' | 'aas_aasx' | 'aas_xml' | 'aas_json' | 'scsn_pcf_xml' | 'scsn_full_xml';

// Fixed Product type to match your existing type (emission_total as string)
interface Product {
  id: string;
  name: string;
  description: string;
  ...
  sku: string;
  supplier: string;
  emission_total: string; // Fixed: string instead of number to match your existing type
}
...
/**
 * Export product data using your backend's built-in export endpoints
 */
 ...
 * This calls your backend API to get emission traces and generates a clean PDF
 */
export async function exportProductPDFReport(companyId: string, product: Product): Promise<void> {
  try {
    // Fetch emission traces from your backend
    const emissionTrace = await apiRequest<any>(`/companies/${companyId}/products/${product.id}/emission_traces/`);
    ...
    let html = `
      <div style="margin-left: ${marginLeft}px; margin-bottom: 15px; border-left: ${depth > 0 ? '2px solid #e5e5e5' : 'none'}; padding-left: ${depth > 0 ? '15px' : '0'};">
        ...
        ${trace.mentions && trace.mentions.length > 0 ? 
          trace.mentions.map((mention: any) => {
            const color = mention.mention_class === 'Error' ? '#dc2626' : 
                         mention.mention_class === 'Warning' ? '#ea580c' : '#059669';
    ...
    <body>
      ...
      ${emissionTrace.mentions && emissionTrace.mentions.length > 0 ? 
        `<div class="section">
          ...
          ${emissionTrace.mentions.map((mention: any) => {
            const color = mention.mention_class === 'Error' ? '#dc2626' : 
                         mention.mention_class === 'Warning' ? '#ea580c' : '#059669';
            return `<div style="color: ${color}; font-weight: bold; margin: 10px 0; padding: 10px; background: rgba(${
              mention.mention_class === 'Error' ? '220, 38, 38' : 
              mention.mention_class === 'Warning' ? '234, 88, 12' : '5, 150, 105'
