// ──────────────────────────────────────────────────────────────
// ProductClientPage – handles product creation/editing interface
// Multi-step form using tabbed navigation with async validation
// ──────────────────────────────────────────────────────────────
"use client";


// ── Custom hook to initialize tab configuration and references ────────────
...
export type DataPassedToTabs = {
    // Unique identifier of the product being created or edited
    productId: string;
    tabKey: string;
    mode: Mode;
...
export default function ProductClientPage() {
    ...
    const {initialTabs, refs} = useTabConfig();
    // ── State: Tab configuration (labels, refs, saved/error/disabled status) ──
    const [tabConfig, setTabConfig] = useState(initialTabs);

    // ── URL params / mode setup ────────────────────────────────────────────────
    // ── State: Tracks product ID and mode (add/edit) ──────────────────────────
    const [productId, setProductId] = useState<string>("");
    const [mode, setMode] = useState<Mode>(Mode.ADD);
    // ── Effect: Parse URL for product_id and set mode on mount ────────────────
    useEffect(() => {
    ...
    // ── Effect: Mark all tabs saved/enabled if in edit mode ───────────────────
    useEffect(() => {
        if (mode === Mode.EDIT) {
            setTabConfig(cfg =>
    ...
    // ── Save logic for first tab (Product Info) ──────────────────────────────
    ...
    const onSaveProductInfoTab = async () => {
        ...
        errorMessage =
            (await (mode === Mode.ADD && !productInfoSavedOnce
                ? refs.productInfoRef.current?.saveTab()
                : refs.productInfoRef.current?.updateTab())) ?? errorMessage;
    ...
    // ── Render main layout and tabbed form ───────────────────────────────────
    return (
        <Suspense
            fallback={
        ...
        "
          ...
          "
                ...
                >
                    ...
                    {/* Stepper Tabs – navigation for each form section */}
                    <TabGroup selectedIndex={activeTab} onChange={setActiveTab}>
                          ...
                          `}
                                    ...
                                    {index < tabConfig.length - 1 && (
                                        <div className="absolute top-4 left-1/2 w-full" aria-hidden="true">
                                            <div
                                                className="h-0.5 bg-gray-400 dark:bg-gray-600 w-full transform translate-x-4"/>
                                        </div>
                                    )}
                                </div>
                            ))}
                        ...
                        {/* Tab Panels – main content for each step */}
                        <TabPanels>
                            {tabConfig.map(t => (
                    ...
                    {/* Error Banner – displays inline errors above content */}
                    ...
                    {/* Navigation Buttons – Back / Save / Next / Finish */}
                    <nav
                        className="
            ...
            "
                    ...
                    {/* Success Modal – shows confirmation after product creation/update */}
                    {showSuccessModal && (
                        <PopupModal
                            title={
