...
export default function ProductListPage() {
  ...
  // Handler for AI advice button clicks in the product table
  // Initiates the AI advice workflow by setting up the necessary state
  // and opening the confirmation step of the modal
  const handleAIButtonClick = (productId: string) => {
    // Find the product by ID to get its name from the products array
    const product = products.find(p => p.id === productId);
    // Store the product ID in state for later use in the AI advice request
    setPendingProductId(productId);
    // Store the product name in state, falling back to empty string if product is undefined
    setPendingProductName(product?.name ?? "");
    // Set the AI modal to the first step ("confirm") to begin the advice workflow
  ...
  // Handles file upload events for product data import
  // Processes both AASX/JSON/XML and CSV/XLSX file formats
  // and delegates to the appropriate helper function
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>, type: "aasx" | "csv") => {
    // Check if user has selected at least one file before proceeding
    if (e.target.files?.[0]) {
  ...
  // Navigate to edit product page of the selected product
  const handleEdit = (id: string) => {
    router.push(`/product-list/product?product_id=${id}`);
  };

  // Export workflow with announcement - Handles clicking the export button for a product
  ...
  // Close export modal and reset selected product
  const handleExportModalClose = () => {
    // Hide the export modal by setting its visibility state to false
    setShowExportModal(false);
    // Clear the selected product from state to prevent memory leaks and reset UI state
    setSelectedProductForExport(null);
  };

  // Row click handler to navigate to product overview
